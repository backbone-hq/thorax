...
use thorax_frontend::{
    ...
    write_current_user_for_root, FrontendError,
};
use thorax_ops::{
...
fn cmd_user_show(cli: &CliContext, args: UserShowArgs) -> Result<ExitCode, FrontendError> {
    ...
    if cli.json {
        ...
        println!("administer: {}", authority.administer);
    }
    Ok(ExitCode::SUCCESS)
}
...
fn cmd_user_invite(cli: &CliContext, args: UserInviteArgs) -> Result<ExitCode, FrontendError> {
    ...
    let bundle_string = encode_invite(prepared.invite())?;
    let bundle_file = match &args.invite_file {
        Some(path) => {
            let bytes = invite_bytes(prepared.invite())?;
    ...
    if cli.json {
        ...
        if args.print_unsafe {
            output["invite"] = json!(bundle_string);
        }
        println!("{output}");
        return Ok(ExitCode::SUCCESS);
    }
    ...
    if !encrypted.is_empty() {
        println!();
        println!(
            "encrypted {} secret(s) so {} can read them",
            encrypted.len(),
            handle_ref(&handle)
        );
    }
