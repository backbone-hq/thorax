...
```sh
f="$(mktemp)" && curl -fsSL -o "$f" "https://github.com/backbone-hq/thorax/releases/download/v1.1.0/install.sh" && echo "2cd1412fcf19b98b785bb844103ec32d1b4c61032bbab4aa06af73c89a8770df  $f" | { command -v sha256sum >/dev/null && sha256sum -c - || shasum -a 256 -c -; } && THORAX_VERSION=v1.1.0 sh "$f"
```
<!-- END PINNED INSTALL -->
...
```sh
# Admin
thorax user invite alice --read app/prod --invite-file alice.thrxi

# Alice
thorax claim alice.thrxi
```

Invites can also be rendered as QR codes. Compact text/QR invites pin the intended vault root and begin rollback protection at claim; `.thrxi` files include the inviter's rollback baseline by default.
